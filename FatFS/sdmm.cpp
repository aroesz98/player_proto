...
#define	CS_H();		sd_spi_chip_select(1U);	/* Set MMC CS "high" */
#define CS_L();		sd_spi_chip_select(0U);	/* Set MMC CS "low" */
...
{
	//sd_spi_send(buff, bc);
	pio_spi_write8_blocking(&spi, buff, bc);
}
...
{
	//sd_spi_receive(buff, bc);
	pio_spi_read8_blocking(&spi, buff, bc);
}
